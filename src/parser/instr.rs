...
pub enum Instr {
    ...
    local_set(LocalIdX) = 0x21,
    local_tee(LocalIdX) = 0x22,
    global_get(GlobalIdX) = 0x23,
    global_set(GlobalIdX) = 0x24,
    table_set(TableIdX) = 0x26,
    i32_load(MemArg) = 0x28,
    i32_store(MemArg) = 0x36,
    i64_store(MemArg) = 0x37,
    call(FuncIdx) = 0x41,
    i64_const(i64) = 0x42,
    i32_add = 0x6a,
    i32_sub = 0x6b,
...
impl Parsable for Instr {
        ...
        Self: std::marker::Sized,
    {
        ...
        Ok(match typ[0] {
            ...
            0x21 => local_set(Parsable::parse(data, stack)?),
            0x22 => local_tee(Parsable::parse(data, stack)?),
            0x23 => global_get(Parsable::parse(data, stack)?),
            0x24 => global_set(Parsable::parse(data, stack)?),
            0x25 => Err(ParseError::UnknownInstruction(Hex(typ)))?,
            0x26 => table_set(Parsable::parse(data, stack)?),
            ...
            0x36 => i32_store(Parsable::parse(data, stack)?),
            0x37 => i64_store(Parsable::parse(data, stack)?),
            ...
            0x42 => i64_const(Parsable::parse(data, stack)?),
